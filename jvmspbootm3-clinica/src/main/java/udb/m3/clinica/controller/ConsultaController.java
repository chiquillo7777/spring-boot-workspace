...
public class ConsultaController {
	...
	Optional<Consulta> consulta = this.servicioConsulta.leerPorId(id);
	ConsultaDTO consultaDTO=null;
	if(consulta.isPresent()) {
			consultaDTO= new ConsultaDTO(
			consulta.get().getIdConsulta(),
			consulta.get().getPaciente().getIdPaciente(),
			consulta.get().getMedico().getIdMedico(),
			consulta.get().getEspecialidad().getIdEspecialidad(),
			consulta.get().getNumConsultorio(),
			consulta.get().getFechaConsulta()
				);
		}
